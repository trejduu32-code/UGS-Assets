class OfferManager {
    ...
    completePurchase(purchase = null) {
        ...
        // Log purchase to Playsaurus SDK (mobile-specific)
        if (purchase && typeof(playsaurusSdk) != "undefined") {
            try {
                var actualPrice = this.currentOffer.rawPrice || (this.currentOffer.price / 100).toString();
                // Normalize comma to period for European locales
                actualPrice = actualPrice.replace(',', '.');
                var actualCurrency = this.currentOffer.currency || "USD";
                var transactionId = purchase.transactionId || purchase.purchaseId || 'unknown';
                
                if (!transactionId && purchase.transactions && purchase.transactions.length > 0) {
                    transactionId = purchase.transactions[0].transactionId || 'unknown';
                }
                
                playsaurusSdk.logPurchase({
                    sku: this.currentOffer.sku,
                    amount: actualPrice,
                    currency: actualCurrency,
                    transactionId: transactionId,
                    productType: "bundle",
                    paymentGateway: platform.isIOs() ? "app_store" : "google_play"
                });
            } catch(e) {
                console.error("Failed to log offer purchase to Playsaurus SDK:", e);
            }
        }
