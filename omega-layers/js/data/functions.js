  ...
  formatNumber: function(n, prec, prec1000, lim = new Decimal(1000)) {
    ...
    if (n.lt(lim)) {
      ...
      return num.toLocaleString("en-us", {
        minimumFractionDigits: prec1000,
        maximumFractionDigits: prec1000
      });
    }
    if (n.layer === 0) {
    ...
    if (n.layer === 1) {
      ...
      return Decimal.pow(10, mag % 1).toFixed(prec) + "e" + Math.floor(mag).toLocaleString("en-us", {
        minimumFractionDigits: 0,
        maximumFractionDigits: 0
      });
    } else {
      return "e".repeat(n.layer) + n.mag.toLocaleString("en-us", {
  ...
  getSaveString() {
    let replacer = function(key, value) {
      if (key === "currentChallenge") {
        return value !== null && value !== undefined && game.layers[value.layer.layer] ? {
          layer: value.layer.layer,
          index: game.layers[value.layer.layer].challenges.findIndex(c => c === value)
        } : null;
      }
      if (key === "settings") {
        return undefined;
      ...
      if (value instanceof PrestigeLayer) {
        return {
          ...
          powerGenerators: value.powerGenerators,
          upgrades: value.upgrades,
          treeUpgrades: value.treeUpgrades,
          power: "d" + value.power,
          resource: "d" + value.resource,
          totalResource: "d" + value.totalResource,
          maxResource: "d" + value.maxResource,
          timeSpent: value.timeSpent,
          timesReset: value.timesReset
        };
      }
      ...
      if (value instanceof Automator) {
        return {
          ...
          active: value.active,
          desiredInterval: value.desiredInterval
        };
      }
      if (value instanceof ReStackLayer) {
        return {
          layerCoins: "d" + value.layerCoins,
          permUpgrades: value.permUpgrades,
          metaUpgrade: value.metaUpgrade,
          upgradeTree: value.upgradeTree,
          timeSpent: value.timeSpent
        };
      }
      if (value instanceof MetaLayer) {
        return {
          ...
          resource: "d" + value.resource,
          layer: "d" + value.layer,
          multiplierUpgrades: value.multiplierUpgrades,
