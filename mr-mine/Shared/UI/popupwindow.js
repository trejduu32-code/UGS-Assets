...
class TabbedPopupWindow extends PopupWindow {
  ...
  initializeFrameImages() {
    if (typeof window !== "undefined" && window["popupBackgroundLight"]) {
      ...
      this.activeTabImage = frameSets[EARTH_INDEX].activeTabImage;
      this.inactiveTabImage = frameSets[EARTH_INDEX].inactiveTabImage;
    }
  }
  ...
  initializeTabs(tabNames) {
    ...
    var sizeMultipleOfPreset =
      maximumTabWidth / (this.tabWidth + this.tabSpacing);
    ...
    var frameYScale =
      popupFrame.height /
      (this.boundingBox.height - this.tabsContainer.boundingBox.y);
    var frameWidth =
      (this.frameWidthFraction * this.boundingBox.width) / uiScaleX;
    var frameHeight =
      (this.frameHeightFraction * this.boundingBox.height) / uiScaleY;
    var frameRightShadowWidth =
      (this.frameRightShadowFraction * this.boundingBox.width) / uiScaleX;
    var frameBottomShadowWidth =
      (this.frameBottomShadowFraction * this.boundingBox.height) / uiScaleY;
    this.tabsContainer.render = function () {
      this.renderChildren();
      var coords = this.getRelativeCoordinates(
        0,
      ...
      context.drawImage(
        ...
        coords.x,
        coords.y - frameOverlap,
        this.boundingBox.width - frameRightShadowWidth,
        this.parent.boundingBox.height -
          coords.y -
          frameBottomShadowWidth +
          frameOverlap,
      );
      ...
      context.drawImage(
        this.parent.popupFrameImage,
        coords.x,
        coords.y - frameOverlap,
        this.boundingBox.width,
        this.parent.boundingBox.height - coords.y + frameOverlap,
      );
    };
    this.bodyContainer = new Hitbox(
      {
        ...
        height:
          this.boundingBox.height -
          this.tabsContainer.boundingBox.height -
          (2 * frameHeight + frameBottomShadowWidth) +
          2 * frameOverlap,
      },
      {},
      "",
    ...
    this.closeButton = this.addHitbox(
      new Button(
        ...
        "",
        {
          x: this.boundingBox.width * 0.94,
          y: this.tabHeight - frameOverlap - this.boundingBox.width * 0.0125,
          width: this.boundingBox.width * 0.05,
          height: this.boundingBox.width * 0.05,
        },
        {
  ...
  createTabRenderFunction(parentElement, tabIndex, tabName) {
    ...
    return function () {
      ...
      if (parentElement.currentTabIndex == tabIndex) {
        context.drawImage(
          parentElement.activeTabImage,
          relativeCoords.x,
          relativeCoords.y,
          this.boundingBox.width * tabImageXScale,
          this.boundingBox.height * tabImageYScale,
        );
        context.fillStyle = "#F2F2F2";
      } else {
        context.drawImage(
          parentElement.inactiveTabImage,
          relativeCoords.x,
          relativeCoords.y,
          this.boundingBox.width * tabImageXScale,
          this.boundingBox.height * tabImageYScale,
      ...
      if (this.flickerStart) {
        if (
          parentElement.currentTabIndex == tabIndex ||
          (this.flickerEnd > 0 &&
            Math.floor(new Date().getTime()) > this.flickerEnd)
        ) {
          this.flickerStart = 0;
        } else {
          var flickerMaxOpacity = 0.35;
          var flickerT =
            ((Math.floor(new Date().getTime()) - this.flickerStart) %
              this.flickerPeriod) /
            (this.flickerPeriod / 2);
          if (flickerT > 1) {
            flickerT = 2 - flickerT;
          }
          context.save();
  ...
  flickerTab(tabIndex, flickerPeriod, maxNumberOfFlashes = 0) {
    ...
    if (maxNumberOfFlashes > 0) {
      tab.flickerEnd =
        Math.floor(new Date().getTime()) + flickerPeriod * maxNumberOfFlashes;
    } else {
      tab.flickerEnd = 0;
    }
  }
  ...
  setFrameImagesByWorldIndex(worldIndex) {
    ...
    this.frameWidth = frameSets[worldIndex].frameWidth;
    this.rightShadowWidth = frameSets[worldIndex].rightShadowWidth;
    this.bottomShadowHeight = frameSets[worldIndex].bottomShadowHeight;
  }
}
