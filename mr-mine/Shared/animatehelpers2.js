...
function renderButton(
  ...
  fillTextShrinkToFit(
    canvas,
    buttonText,
    buttonX + Math.round(buttonW * 0.1),
    buttonY + buttonH / 2,
    Math.round(buttonW * 0.8),
    "center",
    0.15,
  );
  canvas.restore();
...
function setFontToFit(canvas, text, width, height, font) {
  ...
  if (fontMatcher) {
    ...
    if (metrics.width > paddedWidth || textHeight > paddedHeight) {
      widthFontSize = Math.floor(
        fontSize * Math.min(1.0, paddedWidth / metrics.width),
      );
      heightFontSize = Math.floor(
        fontSize * Math.min(1.0, paddedHeight / textHeight),
      );

      fittingFont = fittingFont.replace(
        FONT_SIZE_REGEX,
        Math.min(widthFontSize, heightFontSize) + "px",
      );
    }
  }
...
function renderCheckboxWithText(
  ...
  canvas.fillText(
    text,
    boxX + boxW + boxW * 0.1,
    boxY + boxH / 2 + getCanvasTextHeightCenterOffset(canvas, text),
  );
  canvas.font = previousFont;
  canvas.fillStyle = previousFill;
}
...
var keepCopy = [];
function modifyImageWithNewText(
  image,
  text,
  textFont,
  textFillColor,
  ...
  try {
    ...
    ctx.fillText(
      text,
      textXCenter - ctx.measureText(text).width / 2,
      canvas.height / 2 + textYShift,
    );
    ...
    if (error.name === "SecurityError") {
      console.warn(
        "Cannot modify image with text: Canvas is tainted (cross-origin image). Skipping text overlay.",
        error,
      );
    } else {
      throw error;
...
function drawVerticalImageLoop(
  context,
  image,
  x,
  y,
...
function drawTiledImage(
  ...
  while (tileY < regionY + regionHeight) {
    var fractionalWidth = Math.max(
      1,
      1 - Math.abs(tileX + tileWidth - regionWidth) / tileWidth,
    );
    var fractionalHeight = Math.max(
      1,
      1 - Math.abs(tileY + tileHeight - regionHeight) / tileHeight,
    );
    context.drawImage(
      image,
      0,
...
function setupButtons() {
  ...
  modifyImageWithNewText(
    document.getElementById("cng"),
    _("CREATE"),
    "14px Verdana",
    "#000000",
    3,
    document.getElementById("cng").width / 2,
  );
  modifyImageWithNewText(
    document.getElementById("cngc"),
    _("CANCEL"),
    "14px Verdana",
    "#000000",
    3,
    document.getElementById("cngc").width / 2,
  );
  ...
  document.getElementById("gameNameText").innerHTML = _(
...
function generateShopHtml() {
  return;
  modifyImageWithNewText(
    document.getElementById("buyticketsbutton1"),
    _("BUY"),
    "14px Verdana",
    "#000000",
    3,
    document.getElementById("buyticketsbutton1").width / 2,
  );
  modifyImageWithNewText(
    document.getElementById("buyticketsbutton2"),
    _("BUY"),
    "14px Verdana",
    "#000000",
    3,
  ...
  modifyImageWithNewText(
    document.getElementById("buyticketsbutton3"),
    _("BUY"),
    "14px Verdana",
    "#000000",
    3,
  ...
  modifyImageWithNewText(
    document.getElementById("buyticketsbutton4"),
    _("BUY"),
    "14px Verdana",
    "#000000",
    3,
  ...
  modifyImageWithNewText(
    document.getElementById("buyticketsbutton5"),
    _("BUY"),
    "14px Verdana",
    "#000000",
    3,
  ...
  modifyImageWithNewText(
    document.getElementById("buyticketsbutton6"),
    _("BUY"),
    "14px Verdana",
    "#000000",
    3,
...
var gr = 0.4;
function renderFancyProgressBar(
  context,
  progressBarText,
  fillPercent,
  x,
  ...
  if (frameImage) {
    drawFrame(
      ...
      frameWidth,
      frameWidth,
      frameRightShadowWidth,
      frameBottomShadowHeight,
    );
  }
...
function generateFullMineScreenshot() {
  var canvas = document.createElement("canvas");
  var ctx = canvas.getContext("2d");
  canvas.width = mainw - Math.ceil(mainw * 0.082);
  canvas.height = mainh - mainh * 0.11;
  ...
  while (currentlyViewedDepth < depth - 5) {
    ...
    ctx.drawImage(
      MAIN.canvas,
      Math.ceil(mainw * 0.082),
      Math.ceil(mainh * 0.11) + 1,
      mainw - Math.ceil(mainw * 0.082),
      mainh - (Math.ceil(mainh * 0.11) + 1),
      0,
      0,
      canvas.width,
      canvas.height,
...
function smoothScroll() {
  var scrollToAdd = Math.max(
    Math.min(totalHeight * 0.004, currentScroll * 0.025),
    5,
  );
...
function drawImageRot(
  ...
  ctx.drawImage(
    ...
    imgwidth,
    imgheight,
    xPivotOffset / 2 + (width / 2) * -1,
    yPivotOffset / 2 + (height / 2) * -1,
    width,
    height,
  );
...
function colorSaturate(
  ...
  screenHeight,
  color,
  decimalPercentSaturate,
) {
  canvas.save();
  canvas.globalCompositeOperation = "saturation";
  canvas.fillStyle =
    "hsl(0," + Math.round(decimalPercentSaturate * 100) + "%,50%)"; // set a colour with saturation 0-100
...
function drawContrastedImage(
  ...
  drawImageWithFilters(
    ...
    screenWidth,
    screenHeight,
    "contrast(" + integerContrastPercent + "%)",
  );
}
...
function drawWavy(canvas, img, canvasX, canvasY, canvasWidth, canvasHeight) {
  ...
  for (var y = 0; y < h; y += stepSize) {
    // segment positions
    var lx1 = x1 + Osc(numFramesRendered, y * 0.2, 0.05) * 1.25;
    var lx2 = x2 + Osc(numFramesRendered, y * 0.2, 0.03) * 1.25;
    var lx3 = x3 + Osc(numFramesRendered, y * 0.2, 0.06) * 1.25;
...
function drawChromaShift(
  ...
  if (!chromaShiftChannelData.hasOwnProperty(id)) {
    ...
    CHANNELS.forEach((idx) => {
      chromaShiftChannelData[id][idx] = ctx.createImageData(
        imgData.width,
        imgData.height,
      );
  ...
  CHANNELS.forEach((idx) => {
    ...
    canvas.drawImage(
      ...
      c.width,
      c.height,
      canvasX + channelDrift[idx].x,
      canvasY + channelDrift[idx].y,
      canvasWidth,
      canvasHeight,
    );
  });
...
function updateSparkleCanvas(width, height) {
  ...
  for (let i = 2; i < 36; i++) {
    let oscillationNumber = Math.floor(
      (frameIndex + i) / (oscillationPeriod * 2),
    );
...
function cleanupAnimationFrameCache() {
  if (animationFrameCacheSize > MAX_ANIMATION_CACHE_SIZE) {
    ...
    if (animationFrameCacheSize > MAX_ANIMATION_CACHE_SIZE) {
      ...
      const keysToRemoveMore = remainingKeys.slice(
        0,
        Math.floor(remainingKeys.length / 2),
      );
  ...
  if (!animationFrameCache[frameKey]) {
    ...
    if (getAnimationFrameIndex(32, 10) < 4) {
      ...
      blessedCtx.drawImage(
        shineOverlay,
        (shineOverlay.width / 4) * getAnimationFrameIndex(4, 10),
        0,
...
function drawTintedImage(
  ...
  if (!cachedTintedAssetData.hasOwnProperty(id)) {
    ...
    CHANNELS_INCLUDING_ALPHA.forEach((idx) => {
      cachedTintedAssetData[id][idx] = ctx.createImageData(
        imgData.width,
        imgData.height,
      );
...
//###################### RELIC RENDERING ##########################
function standardRelicRenderFunction(
  canvas,
  relicId,
  canvasX,
  canvasY,
  ...
  if (excavationRewards[relicId].name != "???") {
    ...
    var shadowAlpha = 0.2 + oscillate(numFramesRendered, 8) * 0.22;
...
function warpedRelicRenderFunction(
  canvas,
  relicId,
  canvasX,
  canvasY,
...
function divineRelicRenderFunction(
  canvas,
  relicId,
  canvasX,
  canvasY,
  ...
  drawBlessedEffect(
    ...
    canvasW,
    canvasH,
    excavationRewards[relicId].name,
  );
}
...
function renderChestSliderDots() {
  ...
  for (var i = 0; i < worldClickables.length; i++) {
    if (
      ...
      (worldClickables[i].type == ORANGE_FISH_ID &&
        metalDetectorStructure.level > 4) ||
      ...
      if (metalDetectorStructure.level > 4) {
        ...
        if (timeUntilExpire < 120) {
          ...
          MAIN.globalAlpha =
            0.25 + oscillate(numFramesRendered, flickerRate) * 0.75;
        }
      }
      if (worldClickables[i].type == MINERAL_DEPOSIT_ID) {
        MAIN.drawImage(
          ...
          Math.ceil(
            mainh *
              (0.317 +
                0.49 * (Math.min(worldClickables[i].depth + 2, depth) / depth)),
      ...
      } else if (worldClickables[i].type == EVENT_PROMO_CLICKABLE_ID) {
        MAIN.drawImage(
          promoClickableDot,
          0,
          0,
  ...
  if (metalDetectorStructure.level > 1) {
    ...
    if (
      !chestCacheCanvas ||
      chestService.chests.length !== lastChestCount ||
      window.innerWidth !== lastScreenWidth ||
      window.innerHeight !== lastScreenHeight
    ) {
      ...
      cacheContext.clearRect(
        0,
        0,
        chestCacheCanvas.width,
        chestCacheCanvas.height,
      );
      ...
      for (let i = chestService.chests.length; i > 0; i--) {
        ...
        cacheContext.drawImage(
          ...
          Math.ceil(mainw * 0.0544),
          Math.ceil(
            mainh * (0.3165 + 0.49 * (Math.min(chest.depth, depth) / depth)),
          ) - Math.ceil(mainh * 0.007),
          Math.floor(mainw * 0.0048),
          Math.ceil(mainh * 0.007),
        );
  ...
  if (battleManager.battleWaiting.length > 0) {
    ...
    MAIN.drawImage(
      ...
      Math.ceil(mainw * 0.0544),
      Math.ceil(
        mainh * (0.3165 + 0.49 * (Math.min(battleDepth + 2, depth) / depth)),
      ) - Math.ceil(mainh * 0.007),
      Math.floor(mainw * 0.0048),
      Math.ceil(mainh * 0.007),
    );
...
function drawTradeAsSymbols(context, tradeOffer, x, y, width, height) {
  ...
  var paymentLabel = fillTextWrap(
    ...
    x +
      width / 4 -
      arrowLength / 4 -
      (width / 2 - arrowLength / 2 - 2 * padding) / 2,
    y + padding,
  ...
  context.beginPath();
  context.moveTo(
    x + width / 2 - arrowLength / 2,
    y + height / 2 + arrowHeight / 4,
  ); // Top left of stem
  context.lineTo(
    x + width / 2 + arrowLength / 12,
    y + height / 2 + arrowHeight / 4,
  ); // Top right of stem
  context.lineTo(
    x + width / 2 + arrowLength / 12,
...
function drawTradeIcon(
  ...
  switch (tradeType) {
    case TRADE_TYPE_ORE:
      drawImageFitInBox(
        context,
        worldResources[tradeSubtype].largeIcon,
        x,
        y,
        width,
        height,
    ...
    case TRADE_TYPE_BUFF:
      drawImageFitInBox(
        context,
        buffs.staticBuffs[tradeSubtype].image,
        x,
        y,
        width,
        height,
    ...
    case TRADE_TYPE_RELIC:
      drawImageFitInBox(
        context,
        excavationRewards[tradeSubtype].image,
        x,
        y,
        width,
        height,
    ...
    case TRADE_TYPE_BLUEPRINT:
      drawImageFitInBox(
        context,
        getDrillEquipByBlueprintId(tradeSubtype).icon,
        x,
        y,
        width,
        height,
...
function getImageFromMergedImages(
  cacheId,
  image1,
  image1CropBoundingBox,
  image1DrawBoundingBox,
  image2,
  image2CropBoundingBox,
  image2DrawBoundingBox,
  image3 = null,
  image3CropBoundingBox = null,
  image3DrawBoundingBox = null,
  image4 = null,
  image4CropBoundingBox = null,
  image4DrawBoundingBox = null,
  image5 = null,
  image5CropBoundingBox = null,
  image5DrawBoundingBox = null,
) {
  if (!assetCache.exists(cacheId)) {
    let tempCanvas = document.createElement("canvas");
    let tempContext = tempCanvas.getContext("2d");
    tempCanvas.width = Math.max(
      image1DrawBoundingBox.x + image1DrawBoundingBox.width,
      image2DrawBoundingBox.x + image2DrawBoundingBox.width,
    );
    tempCanvas.height = Math.max(
      image1DrawBoundingBox.y + image1DrawBoundingBox.height,
      image2DrawBoundingBox.y + image2DrawBoundingBox.height,
    );
    if (image3 != null) {
      tempCanvas.width = Math.max(
        tempCanvas.width,
        image3DrawBoundingBox.x + image3DrawBoundingBox.width,
      );
      tempCanvas.height = Math.max(
        tempCanvas.height,
        image3DrawBoundingBox.y + image3DrawBoundingBox.height,
      );
    }
    if (image4 != null) {
      tempCanvas.width = Math.max(
        tempCanvas.width,
        image4DrawBoundingBox.x + image4DrawBoundingBox.width,
      );
      tempCanvas.height = Math.max(
        tempCanvas.height,
        image4DrawBoundingBox.y + image4DrawBoundingBox.height,
      );
    }
    if (image5 != null) {
      tempCanvas.width = Math.max(
        tempCanvas.width,
        image5DrawBoundingBox.x + image5DrawBoundingBox.width,
      );
      tempCanvas.height = Math.max(
        tempCanvas.height,
        image5DrawBoundingBox.y + image5DrawBoundingBox.height,
      );
    }

    tempContext.drawImage(
      image1,
      image1CropBoundingBox.x,
      image1CropBoundingBox.y,
      image1CropBoundingBox.width,
      image1CropBoundingBox.height,
      image1DrawBoundingBox.x,
      image1DrawBoundingBox.y,
      image1DrawBoundingBox.width,
      image1DrawBoundingBox.height,
    );
    tempContext.drawImage(
      image2,
      image2CropBoundingBox.x,
      image2CropBoundingBox.y,
      image2CropBoundingBox.width,
      image2CropBoundingBox.height,
      image2DrawBoundingBox.x,
      image2DrawBoundingBox.y,
      image2DrawBoundingBox.width,
      image2DrawBoundingBox.height,
    );
    if (image3 != null) {
      tempContext.drawImage(
        image3,
        image3CropBoundingBox.x,
        image3CropBoundingBox.y,
        image3CropBoundingBox.width,
        image3CropBoundingBox.height,
        image3DrawBoundingBox.x,
        image3DrawBoundingBox.y,
        image3DrawBoundingBox.width,
        image3DrawBoundingBox.height,
      );
    }
    if (image4 != null) {
      tempContext.drawImage(
        image4,
        image4CropBoundingBox.x,
        image4CropBoundingBox.y,
        image4CropBoundingBox.width,
        image4CropBoundingBox.height,
        image4DrawBoundingBox.x,
        image4DrawBoundingBox.y,
        image4DrawBoundingBox.width,
        image4DrawBoundingBox.height,
      );
    }
    if (image5 != null) {
      tempContext.drawImage(
        image5,
        image5CropBoundingBox.x,
        image5CropBoundingBox.y,
        image5CropBoundingBox.width,
        image5CropBoundingBox.height,
        image5DrawBoundingBox.x,
        image5DrawBoundingBox.y,
        image5DrawBoundingBox.width,
        image5DrawBoundingBox.height,
      );
    }
...
function renderRoundedRectangle(
  canvas,
  x,
  y,
  width,
...
function generateIconWithCooldownOverlay(
  icon,
  width,
  height,
  remainingFraction,
...
function drawFrame(
  ...
  context.drawImage(
    ...
    baseFrameWidth,
    0,
    frameImage.width - 2 * baseFrameWidth - rightShadowWidth,
    baseFrameWidth,
    x + frameWidth,
    y,
    width - 2 * frameWidth,
  ...
  context.drawImage(
    frameImage,
    frameImage.width - baseFrameWidth - rightShadowWidth,
    0,
    baseFrameWidth + rightShadowWidth,
    baseFrameWidth,
    x + width - frameWidth,
    y,
    frameWidth + rightShadowWidth * scale,
    frameWidth,
  );
  // right
  context.drawImage(
    ...
    baseFrameWidth,
    baseFrameWidth + rightShadowWidth,
    frameImage.height - 2 * baseFrameWidth - bottomShadowHeight,
    x + width - frameWidth,
    y + frameWidth,
    frameWidth + rightShadowWidth * scale,
    height - 2 * frameWidth,
  ...
  context.drawImage(
    frameImage,
    frameImage.width - baseFrameWidth - rightShadowWidth,
    frameImage.height - baseFrameWidth - bottomShadowHeight,
    baseFrameWidth + rightShadowWidth,
    baseFrameWidth + bottomShadowHeight,
    x + width - frameWidth,
    y + height - frameWidth,
    frameWidth + rightShadowWidth * scale,
    frameWidth + bottomShadowHeight * scale,
  );
  // bottom
  context.drawImage(
    frameImage,
...
function drawNineSlice(
  ...
  var rCornerScale = Math.min(
    hScale,
    vScale,
    scaleCorners ? Number.MAX_SAFE_INTEGER : 1,
  );
...
function getAnimationFrameIndex(
  ...
  return (
    Math.floor(
      (phaseShift + numFramesRendered) * (framerate / IDLE_FRAMERATE),
    ) % framesInAnimation
  );
}
 ...
 */
function drawCachedHintHighlight(
  canvas,
  x,
  y,
  width,
  ...
  if (!isMobile()) {
    ...
    canvas.shadowBlur =
      ...
      (maxAlpha - minAlpha) *
        highlightBlur *
        oscillate(phaseShift + numFramesRendered, pulsePeriod / 2);
