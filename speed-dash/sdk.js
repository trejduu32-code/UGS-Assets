! function(t) {
  ...
  var tt = function() {
      console.log("PokiSDK: Bypassing external config fetch (tt function).");
      // Return a promise that resolves immediately with 'undefined'.
      // This prevents the network request and provides a neutral value
      // for the .then() block in lazyLoadInit that expects a config object.
  ...
  function it() {
    // --- Original Fetch Code Removed ---
    // Prevent fetching lng.txt and potential JSON parse errors.
    // Directly return a default value (e.g., "ZZ" which was the original fallback).
    // Using the async generator return syntax [2, value].
    return ot(this, void 0, Promise, (function() {
      return et(this, (function(n) {
        console.log("PokiSDK: Bypassing lng.txt fetch, returning 'ZZ'.");
        return [2, "ZZ"]; // Directly return the fallback value
      }))
    }));
  }
  var rt = e(67),
    ...
    mt = function() {
      ...
      return t.prototype.init = function(t) {
        ...
        // --- Prebid Initialization Promise (kept as is, unless prebid is also not needed) ---
        ...
        var H = tt; // Function to get game config
        (t.isPokiPlatform || this.debug) && (H = function() {
          // On platform or debug, don't need to fetch config externally
        ...
        var N = S("pokiDebug");
        N ? (this.setDebug("true" === N), this.debugIsOverwritten = !0) : this.setDebug(this.debug || u);
        this.debugTouchOverlayController && (v() || b()) && dt();

        // --- Modified Promise.all ---
        // Removed J() calls for ima3.js and gpt.js, replaced with Promise.resolve()
        // to prevent loading external ad libraries.
        return Promise.all([
          H(), // Keep fetching game config (or resolves if debug/platform)
          it(), // Keep fetching language config (or returns "ZZ" after previous modification)
          Promise.resolve(), // Replaced J("https://imasdk.googleapis.comsa/js/sdkloader/ima3.js")
          Promise.resolve(), // Replaced J("https://securepubads.g.doubleclick.netsa/tag/js/gpt.js")
          Z // Keep Prebid promise if needed
        ]).catch((function(err) { // Enhanced catch for robustness

          l.dispatchEvent(c.adblocked); // Dispatch adblocked event on any critical init failure
          // Resolve with a placeholder array matching the original structure to prevent breaking the .then chain
          // Adjust placeholders based on what the .then block strictly requires (e[0] and e[1] seem important).
          return [undefined, "ZZ", undefined, undefined, undefined];
        })).then((function(e) {
          // The rest of the .then block remains the same, but now it executes even if ad libs were blocked/skipped.
          // It relies on e[0] (game config result) and e[1] (language result).
          if (void 0 !== e) { // Ensure e is defined (it might be undefined if catch didn't return anything)
            var i = e[0], // Result from H()
              r = e[1], // Result from it()
              o = ct(ct({}, G), {
                country: r // Use the result from it()
              });
            if (i) { // If game config was fetched successfully
              lt.gameId || (lt.gameId = i.gameId);
              ["7fb1fd45-24ce-4ade-b5c4-9ee55ec99526"].includes(lt.gameId) && (v() || b()) && dt();
            ...
            // Setup Player Skin based on flags
            if ((y || x) && t.isPokiPlatform) {
              n.playerSkin = new F({
                wrapper: g,
                minimizeFeature: x,
                skipFeature: y,
            ...
            } else if (t.GetIsSkipTestCountry()) {
              n.playerSkin = new F({
                wrapper: g,
                minimizeFeature: !1,
                skipFeature: !0,
            ...
            // Initialize IMA only if it seems necessary (though we bypassed loading, maybe internal refs exist)
            // Safely attempt to create IMA instance
            try {
              n.ima = new K; // This might still fail if IMA wasn't loaded, handle potential error
            } catch (imaError) {
              console.warn("PokiSDK: Failed to initialize IMA instance, likely due to skipped loading.", imaError);
              n.sdkImaError = true; // Mark IMA as errored/unavailable
              n.ima = null; // Ensure ima is null if it failed
            }

            if (n.ima) { // Only setup IMA related things if it was successfully instantiated
              ...
              // Handle case where IMA isn't available (e.g., skip waterfall setup or use a dummy)
              console.warn("PokiSDK: IMA not available, ad waterfall functionality might be limited.");
              n.waterfall = null; // Ensure waterfall is null
            }


            // Attempt to setup googletag safely
            try {
              n.googletag = window.googletag || {
                cmd: []
              }; // Assign potentially existing or new command queue
            } catch (gtagError) {
              console.warn("PokiSDK: Failed to initialize googletag instance, likely due to skipped loading.", gtagError);
              n.googletag = {
                cmd: []
              }; // Ensure it's a dummy object if failed
            }


            n.isInitialized = !0;
            l.dispatchEvent(c.ready); // Dispatch ready event
          } else {
            console.error("PokiSDK: Initialization failed critically after Promise.all catch.");
            // Dispatch adblocked or a specific init failed event if not already done
            if (!n.sdkBooted) { // Check if adblocked was already dispatched
    ...
    zt = function() {
      function t() {
        ...
        this.gameStarted = !1, this.SDK = new pt, this.init = function(n) {
          ...
          return new Promise((function(n) { // n is the resolve function
            // Track the event as if the break was initiated
            g.track(c.tracking.screen.commercialBreak);
            console.log("PokiSDK: Bypassing commercialBreak, resolving immediately.");

            // Immediately resolve the promise, indicating the break is finished.
            // No ad is requested or shown.
            n(); // Resolve the promise immediately
          }));
        }, this.rewardedBreak = function(onStart) { // Parameter name is onStart
          return new Promise((resolvePromise) => {
            // --- GPT Availability Check ---
            if (!window.googletag || !window.googletag.defineOutOfPageSlot) {
              console.error("PokiSDK: Google Publisher Tag (GPT) is not available for rewardedBreak.");
              showFallbackBannerAndRewardAfterDelay("gpt_missing_for_rewarded_video");
              return;
            }

            window.googletag.cmd = window.googletag.cmd || [];

            let rewardedSlotAdInstance;
            let shouldGrantReward = false;
            let adPlayOrContentStallTimeoutId = null; // For initial ad content rendering
            const AD_CONTENT_STALL_TIMEOUT_MS = 15000; // Configurable: Timeout for initial ad render after slotReady

            let masterOperationTimeoutId = null; // For overall ad request before slotReady
            const MASTER_OPERATION_TIMEOUT_MS = 15000;

            let promiseResolved = false;
            let slotReadyFiredOnce = false;

            // UI Element IDs
            const LOADING_SCREEN_ID = 'pokiAdLoadingScreenInternal';
            const REWARD_MESSAGE_SCREEN_ID = 'pokiRewardMessageScreenInternal';
            const FALLBACK_BANNER_CONTAINER_ID = 'pokiFallbackBannerContainerInternal';
            const FALLBACK_BANNER_CLOSE_BUTTON_ID = 'pokiFallbackBannerCloseButtonInternal';
            const FALLBACK_MESSAGE_ID = 'pokiFallbackMessageInternal';

            // Timers for fallback banner
            let showCloseButtonTimerId = null;
            let fallbackCountdownIntervalId = null;
            const FALLBACK_COUNTDOWN_SECONDS = 5;


            function logStatus(message) {
              console.log('PokiSDK rewardedBreak: ', message);
            }

            // --- Dynamic UI Helper Functions ---
            function getOrCreateUiElement(id, innerHTMLContent, tag = 'div') {
              let element = document.getElementById(id);
              if (!element) {
                element = document.createElement(tag);
                element.id = id;
                document.body.appendChild(element);
                logStatus(`UI HINT: Dynamically created UI element #${id}.`);
              }
            ...
            function showAdLoadingUI() {
              hideAllAdUIElements();
              const loadingElement = getOrCreateUiElement(LOADING_SCREEN_ID, "Ad is loading...<br><small style='font-size:0.7em'>Please wait for your reward!</small>");
              ...
              logStatus("UI HINT: SDK 'Ad is loading' UI shown.");
            }

            function showRewardMessageUI(grantReward, reason) {
              hideAllAdUIElements();
              const rewardElement = getOrCreateUiElement(REWARD_MESSAGE_SCREEN_ID, "");
              if (grantReward) {
                rewardElement.innerHTML = "You have been rewarded!<br><small style='font-size:0.7em'>(Resuming game shortly...)</small>";
              } else {
                rewardElement.innerHTML = `Ad finished. No reward this time.<br><small style='font-size:0.7em'>(Reason: ${reason || 'Not completed'})</small>`;
              }
              Object.assign(rewardElement.style, {
                top: '0',
                left: '0',
                width: '100vw',
                height: '100vh',
              ...
              logStatus(`UI HINT: SDK Reward/End message UI shown (reward: ${grantReward}).`);
              setTimeout(hideAllAdUIElements, 2000); // Message shown for 2 seconds
            }

            function hideAdLoadingUI() {
              const loadingElement = document.getElementById(LOADING_SCREEN_ID);
              if (loadingElement) loadingElement.style.display = 'none';
            }

            function hideFallbackElements() {
              const bannerContainer = document.getElementById(FALLBACK_BANNER_CONTAINER_ID);
              if (bannerContainer) bannerContainer.style.display = 'none';

              const closeButton = document.getElementById(FALLBACK_BANNER_CLOSE_BUTTON_ID);
              if (closeButton) closeButton.style.display = 'none';

              const fallbackMessage = document.getElementById(FALLBACK_MESSAGE_ID);
              if (fallbackMessage) fallbackMessage.style.display = 'none';

              if (fallbackCountdownIntervalId) clearInterval(fallbackCountdownIntervalId);
              fallbackCountdownIntervalId = null;
              if (showCloseButtonTimerId) clearTimeout(showCloseButtonTimerId);
              showCloseButtonTimerId = null;
            }

            function hideAllAdUIElements() {
              hideAdLoadingUI();
              hideFallbackElements();
              const rewardElement = document.getElementById(REWARD_MESSAGE_SCREEN_ID);
              if (rewardElement) rewardElement.style.display = 'none';
              logStatus("UI HINT: SDK All ad-related UI elements potentially hidden.");
            }

            // --- MODIFIED SECTION within showFallbackBannerAndRewardAfterDelay ---
            function showFallbackBannerAndRewardAfterDelay(failureReason = "unknown_rewarded_video_failure") {
              if (promiseResolved) return;

              logStatus(`Fallback: Rewarded video ad not available or failed (Reason: ${failureReason}). Attempting to show fallback AdSense banner.`);
              hideAdLoadingUI();

              const bannerContainer = getOrCreateUiElement(FALLBACK_BANNER_CONTAINER_ID, null);
              Object.assign(bannerContainer.style, {
                top: '0',
                left: '0',
                width: '100vw',
                height: '100vh',
                backgroundColor: 'rgba(0, 0, 0, 0.9)', // This was already in your provided code
                display: 'flex',
                flexDirection: 'column',
                justifyContent: 'center',
                alignItems: 'center',
                padding: '20px', // Added padding for overall container
                boxSizing: 'border-box' // Added for consistent layout
              });

              const fallbackMessageElement = getOrCreateUiElement(FALLBACK_MESSAGE_ID, "");
              // --- Styles for the timer message element ---
              Object.assign(fallbackMessageElement.style, {
                display: 'block', // Makes it a block element, centered by parent's alignItems
                backgroundColor: 'rgba(20, 20, 20, 0.85)', // Dark background FOR THE TEXT MESSAGE
                color: 'white', // White text color
                padding: '10px 15px', // Padding inside the message box
                borderRadius: '5px', // Rounded corners for the message box
                fontSize: 'clamp(16px, 2.5vw, 20px)', // Responsive font size
                fontWeight: 'bold',
                textAlign: 'center',
                marginBottom: '15px', // Space between message and ad banner below
                maxWidth: '90%', // Max width of the message box
                boxSizing: 'border-box', // Include padding in width calculation
                lineHeight: '1.4' // Line height for readability
              });
              // --- End of styles for the timer message element ---

              const adWrapper = document.createElement('div');
              // Styles for adWrapper are taken from your provided code
              Object.assign(adWrapper.style, {
                backgroundColor: 'white',
                padding: '10px',
                borderRadius: '5px',
                display: 'inline-block',
                boxShadow: '0 0 15px rgba(0,0,0,0.5)', // As per your provided code
                position: 'relative'
              });
            ...
            `;
              ...
              bannerContainer.appendChild(fallbackMessageElement); // Message first
              bannerContainer.appendChild(adWrapper); // Then ad wrapper

              const closeButton = getOrCreateUiElement(FALLBACK_BANNER_CLOSE_BUTTON_ID, '&#x2716;', 'button');
              // Styles for closeButton are taken from your provided code
              ...
              try {
                if (window.adsbygoogle && typeof window.adsbygoogle.push === 'function') {
                  ...
                  logStatus("AdSense push({}) called for fallback banner.");
                } else {
                  logStatus("AdSense (adsbygoogle.js or push function) not found. Fallback banner may not load.");
                }
              } catch (e) {
                logStatus(`Error pushing to adsbygoogle: ${e.message}`);
              }

              let countdown = FALLBACK_COUNTDOWN_SECONDS;
              fallbackMessageElement.innerHTML = `You can close this ad in ${countdown} seconds.`;

              if (fallbackCountdownIntervalId) clearInterval(fallbackCountdownIntervalId);
              fallbackCountdownIntervalId = setInterval(() => {
                countdown--;
                if (countdown > 0) {
                  fallbackMessageElement.innerHTML = `You can close this ad in ${countdown} seconds.`;
                } else {
                  clearInterval(fallbackCountdownIntervalId);
                  fallbackCountdownIntervalId = null;
                  fallbackMessageElement.innerHTML = "You can close this ad now.";
                  if (!promiseResolved) {
                    logStatus("Fallback: Countdown finished. Showing close button for banner.");
              ...
              if (showCloseButtonTimerId) clearTimeout(showCloseButtonTimerId);
              showCloseButtonTimerId = setTimeout(() => {
                if (promiseResolved || (fallbackCountdownIntervalId === null && closeButton.style.display === 'flex')) return;
                logStatus("Fallback: Backup timer ensuring close button is shown.");
                closeButton.style.display = 'flex';
                if (fallbackCountdownIntervalId) {
                  clearInterval(fallbackCountdownIntervalId);
                  fallbackCountdownIntervalId = null;
                  fallbackMessageElement.innerHTML = "You can close this ad now.";
                }
              }, (FALLBACK_COUNTDOWN_SECONDS * 1000) + 500);

              const closeBannerAndGrantReward = () => {
                if (promiseResolved) return;
                logStatus("Fallback: Close button clicked for banner.");
                ...
                finalResolve(true, "fallback_banner_closed_by_user");
              };
              closeButton.onclick = closeBannerAndGrantReward;
            ...
            function finalResolve(grantReward, reason) {
              if (promiseResolved) {
                logStatus(`Promise already resolved. Attempt (reason: ${reason}, reward: ${grantReward}) ignored.`);
                return;
              }
              promiseResolved = true;
              logStatus(`Finalizing. Reason: "${reason}". Granting reward: ${grantReward}.`);

              // Clear ALL outstanding timers
              if (adPlayOrContentStallTimeoutId) clearTimeout(adPlayOrContentStallTimeoutId);
              adPlayOrContentStallTimeoutId = null;
              if (masterOperationTimeoutId) clearTimeout(masterOperationTimeoutId);
              masterOperationTimeoutId = null;
              if (showCloseButtonTimerId) clearTimeout(showCloseButtonTimerId);
              showCloseButtonTimerId = null;
              if (fallbackCountdownIntervalId) clearInterval(fallbackCountdownIntervalId);
              fallbackCountdownIntervalId = null;

              if (rewardedSlotAdInstance && window.googletag && window.googletag.destroySlots) {
                try {
                  googletag.destroySlots([rewardedSlotAdInstance]);
                  rewardedSlotAdInstance = null;
                  logStatus('GPT slot instance destroyed by SDK.');
                } catch (e) {
                  logStatus(`Error destroying GPT slot by SDK: ${e.message}`);
                }
              }
              showRewardMessageUI(grantReward, reason);
              resolvePromise(grantReward);
            }

            googletag.cmd.push(() => {
              const adUnitPath = '/23189367173/PAPASGAMING.COMmviE/PAPASGAMING.COM_REWARDEDmvJR';

              const existingSlots = googletag.pubads().getSlots();
              existingSlots.forEach(slot => {
                if (slot.getAdUnitPath() === adUnitPath) {
                  logStatus('SDK: Destroying pre-existing/conflicting slot: ' + slot.getSlotElementId());
                  try {
                    googletag.destroySlots([slot]);
                  } catch (e) {
                    logStatus(`SDK: Error destroying pre-existing slot: ${e.message}`);
                  }
                }
              });

              rewardedSlotAdInstance = googletag.defineOutOfPageSlot(adUnitPath, googletag.enums.OutOfPageFormat.REWARDED);

              if (rewardedSlotAdInstance) {
                rewardedSlotAdInstance.addService(googletag.pubads());

                googletag.pubads().addEventListener('rewardedSlotReady', (event) => {
                  if (event.slot !== rewardedSlotAdInstance || slotReadyFiredOnce || promiseResolved) return;
                  slotReadyFiredOnce = true;
                  logStatus('SDK GPT Event: rewardedSlotReady.');
                  if (masterOperationTimeoutId) {
                    clearTimeout(masterOperationTimeoutId);
                    masterOperationTimeoutId = null;
                  }
                  ...
                  if (typeof onStart === 'function') {
                    try {
                      onStart();
                      logStatus('SDK: External onStart callback executed.');
                    } catch (e) {
                      logStatus(`SDK: Error in external onStart callback: ${e.message}`);
                    }
                  }

                  if (adPlayOrContentStallTimeoutId) clearTimeout(adPlayOrContentStallTimeoutId);
                  adPlayOrContentStallTimeoutId = setTimeout(() => {
                    logStatus(`SDK: Initial ad content did not render/report within ${AD_CONTENT_STALL_TIMEOUT_MS/1000}s after slotReady.`);
                    if (!promiseResolved) {
                      showFallbackBannerAndRewardAfterDelay(`rewarded_video_initial_render_timeout_${AD_CONTENT_STALL_TIMEOUT_MS/1000}s`);
                    }
                  }, AD_CONTENT_STALL_TIMEOUT_MS);

                  try {
                    event.makeRewardedVisible();
                    logStatus('SDK GPT Event: makeRewardedVisible() called.');
                  } catch (e) {
                    logStatus(`SDK Error calling makeRewardedVisible: ${e.message}`);
                    if (!promiseResolved) {
                      showFallbackBannerAndRewardAfterDelay("make_rewarded_visible_error");
                    }
                  }
                });

                googletag.pubads().addEventListener('rewardedSlotClosed', (event) => {
                  if (event.slot !== rewardedSlotAdInstance || promiseResolved) return;
                  logStatus('SDK GPT Event: rewardedSlotClosed.');
                  if (adPlayOrContentStallTimeoutId) {
                    logStatus('SDK: Clearing adPlayOrContentStallTimeoutId on rewardedSlotClosed.');
                    clearTimeout(adPlayOrContentStallTimeoutId);
                    adPlayOrContentStallTimeoutId = null;
                  }
                  finalResolve(shouldGrantReward, "slot_closed_naturally");
                });

                googletag.pubads().addEventListener('rewardedSlotGranted', (event) => {
                  if (event.slot !== rewardedSlotAdInstance || promiseResolved) return;
                  logStatus('SDK GPT Event: rewardedSlotGranted. User will be rewarded.');
                  shouldGrantReward = true;
                  if (adPlayOrContentStallTimeoutId) {
                    logStatus('SDK: Clearing adPlayOrContentStallTimeoutId on rewardedSlotGranted.');
                    clearTimeout(adPlayOrContentStallTimeoutId);
                    adPlayOrContentStallTimeoutId = null;
                  }
                });
                ...
                googletag.pubads().addEventListener('slotRenderEnded', (event) => {
                  ...
                  if (event.isEmpty) {
                    logStatus('SDK GPT Event: slotRenderEnded (isEmpty=true). No rewarded video. Triggering fallback banner.');
                    if (adPlayOrContentStallTimeoutId) {
                      clearTimeout(adPlayOrContentStallTimeoutId);
                      adPlayOrContentStallTimeoutId = null;
                    }
                    showFallbackBannerAndRewardAfterDelay("rewarded_video_empty");
                  } else {
                    logStatus('SDK GPT Event: slotRenderEnded (not empty). Rewarded video creative was fetched.');
                    if (adPlayOrContentStallTimeoutId) {
                      logStatus('SDK: Ad creative fetched (not empty), clearing initial adPlayOrContentStallTimeoutId.');
                      clearTimeout(adPlayOrContentStallTimeoutId);
                      adPlayOrContentStallTimeoutId = null;
                    }
                  }
                ...
                try {
                  ...
                  logStatus('SDK: googletag.display() called to request the ad.');
                  googletag.display(rewardedSlotAdInstance);
                } catch (e) {
                  logStatus(`SDK Error during googletag.enableServices() or display(): ${e.message}`);
                  if (!promiseResolved) {
                    showFallbackBannerAndRewardAfterDelay("gpt_display_error");
                  }
                  return;
                }

                if (masterOperationTimeoutId) clearTimeout(masterOperationTimeoutId);
                masterOperationTimeoutId = setTimeout(() => {
                  logStatus(`SDK: Master ${MASTER_OPERATION_TIMEOUT_MS/1000}s operation timeout (pre-ready stall). Assuming no rewarded video, try fallback.`);
                  if (!promiseResolved) {
                    showFallbackBannerAndRewardAfterDelay(`master_timeout_pre_ready_${MASTER_OPERATION_TIMEOUT_MS/1000}s`);
                  }
                }, MASTER_OPERATION_TIMEOUT_MS);

              } else {
                logStatus('SDK: Rewarded ad slot could not be defined by GPT.');
                if (!promiseResolved) {
                  showFallbackBannerAndRewardAfterDelay("slot_definition_failed");
                }
              }
            });
          });
