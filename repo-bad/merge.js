(function () {
  ...
  function shouldInterceptFile(url) {
    ...
    for (const file of config.files) {
      ...
      // Strip compression suffixes for matching
      const urlWithoutCompression = urlStr
        .replace(/\.gz$/, "")
        .replace(/\.br$/, "");
      ...
      for (const pattern of [
        ...
        if (
          urlsMatch(urlWithoutCompression, pattern) ||
          urlsMatch(urlStr, pattern)
        ) {
          log(
            "Matched file:",
            fileName,
  ...
  window.fetch = function (url, ...args) {
    ...
    if (filename) {
      log("Intercepting fetch for:", url, "-> mapped to:", filename);
      ...
      return new Promise((resolve, reject) => {
        ...
        const check = setInterval(() => {
          ...
          if (buffer) {
            ...
            // Don't set Content-Encoding if we're returning raw uncompressed data
            // The parts are pre-split but not pre-compressed

            log(
              "Resolved intercepted fetch for:",
              filename,
              "size:",
              buffer.byteLength,
            );
            ...
            clearInterval(check);
            error(`Merge failed for ${filename}`);
            reject(new Error(`Merge failed for ${filename}`));
          } else if (Date.now() - startTime > maxWait) {
            clearInterval(check);
            error(`Timeout waiting for ${filename}`);
  ...
  window.XMLHttpRequest = function (options) {
    ...
    xhr.send = function (...args) {
      ...
      if (filename) {
        log("Intercepting XHR for:", requestUrl, "-> mapped to:", filename);
  ...
  function init() {
    log("Initializing file merger...");
    log("Config:", config);

    if (document.body) {
      log("Document body exists, initializing UI");
      initializeUI();
      autoMergeFiles();
    } else {
      ...
      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", () => {
          log("DOMContentLoaded fired, initializing UI");
          initializeUI();
          autoMergeFiles();
        });
      } else {
