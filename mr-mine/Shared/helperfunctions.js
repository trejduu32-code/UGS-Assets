 ...
 * @returns {number} Random number between min and max
 */
function biasedRand(min, max, probabilityPower = 0.5) {
  return Math.floor(
    min + (max + 1 - min) * Math.pow(Math.random(), probabilityPower),
  );
}
...
function divideBigNumberToDecimalNumber(
  numeratorBigNumber,
  denominatorBigNumber,
  decimalPlaces = 3,
) {
  return new BigNumber(numeratorBigNumber)
    .divide(denominatorBigNumber)
    .toFloat();
}
...
function shortenAndBeautifyNum(n, precision = 1) {
  ...
  for (var index = 0; index < NUMBER_SUFFIXES.length * 3; index++) {
    if (n < Math.pow(10, index * 3 + 3)) {
      ...
      return (
        n.toFixed(precision).replace("." + "0".repeat(precision), "") +
        NUMBER_SUFFIXES[index]
      );
    }
  }
}
...
function getInterpolatedColorArray(
  startColorHex,
  startAlpha,
  endColorHex,
  endAlpha,
  numStepsIncludingStartAndEnd,
) {
  var result = [];
  ...
  for (var i = 0; i <= numStepsIncludingStartAndEnd; i++) {
    ...
    var aPoint =
      Math.round((startRgba.a + colorPointMultiplier * aDifference) * 100) /
      100;
 ...
 */
function fillTextWrapWithHeightLimit(
  context,
  text,
  x,
  y,
  ...
  if (!textSizeCache.exists(textCacheKey)) {
    ...
    var maximumFontSizePossible =
      (fontSize * 3 * maxHeight) / (estimatedLines * lineHeight);
    ...
    if (
      fillTextWrap(context, text, x, y, maxWidth, justify, lineSpacing, true)
        .height > maxHeight
    ) {
      let minSize = 1;
 ...
 * @returns {object} Object containing the properties of the text
 */
function strokeTextWrapWithHeightLimit(
  context,
  text,
  x,
  y,
 ...
 * @returns {object} Object containing the properties of the text
 */
function outlineTextWrapWithHeightLimit(
  context,
  text,
  x,
  y,
  ...
  if (verticalJustify == "top") {
    ...
    var dimensions = outlineTextWrap(
      context,
      text,
      x,
      y,
...
function drawTextWrap(
  ...
  for (var i = 0; i < words.length; ++i) {
    ...
    if (wordWidth > maxWidth) {
      ...
      wordWidth =
        context.measureText(words[i]).width + (i > 0 ? spaceWidth : 0);
    }
    ...
    if (wordWidth > maxWidth || boldText) {
      var newFontSize =
        wordWidth > maxWidth ? (fontSize * maxWidth) / lineWidth : fontSize;
...
function strokeTextShrinkToFit(
  context,
  text,
  x,
  y,
 ...
 */
function outlineTextShrinkToFit(
  context,
  text,
  x,
  y,
...
function generateExternalLinkHtml(link, anchor) {
  return (
    "<span style='cursor: pointer; color: #6666FF; text-decoration: underline;' onClick='openExternalLinkInDefaultBrowser(\"" +
    link +
    "\")'>" +
    anchor +
    "</span>"
                ...
                function (response) {
                  if (response) {
                    ...
                    if (response.success) {
                      ...
                      var minerDepth =
                        startDepthForMinerNames +
                        Math.floor(response.currentNameCount / 10);
...
function takeScreenshotsForAllLanguages() {
  ...
  if (remainingLanguages.length === 0) {
    console.warn(
      "No languages available in 'translations' to take screenshots for.",
    );
    return;
  }

  console.log(
    `Starting screenshots for ${remainingLanguages.length} languages.`,
  );
...
function takeScreenshotForLanguage() {
  ...
  ensureFolderExists(folder)
    ...
    .catch((err) => {
      console.error(
        `Error processing language ${currentLanguage}: ${err.message}`,
      );
    });
}
...
function takeScreenshot(fileName) {
  return new Promise((resolve, reject) => {
    try {
      ...
      setTimeout(() => ipcRenderer.send("take-screenshot", fileName), 200);
    } catch (err) {
      reject(err);
    }
  });
...
function generateNameStubs(numNames, nameArray = null) {
  var result = "";
  var currentDepth =
    paidMinerNames[paidMinerNames.length - 1].depth + rand(1, 2);
  var previousWorkerLocation = 1;
  var i = 0;
  while (i < numNames) {
    if (
      !isDepthWithoutWorkers(currentDepth) &&
      !isDepthWithoutWorkers(currentDepth - 1) &&
      !isDepthWithoutWorkers(currentDepth + 1)
    ) {
    ...
    while (i < numNames) {
      if (
        ...
        if (paidMinersAtDepth < 3) {
          var newWorkerLocation = rand(1, 9);
          while (
            newWorkerLocation == previousWorkerLocation ||
            getPremiumMinerName(currentDepth, newWorkerLocation) != null
          ) {
            newWorkerLocation = rand(1, 9);
          }
          previousWorkerLocation = newWorkerLocation;
...
function runScenes() {
  ...
  var scene3 =
    "function(){generateAnimatedSequence(240, 360, 980, 990, 25, " +
    scene4 +
    ");}";
  var scene2 =
    "function(){generateAnimatedSequence(120, 240, 293, 300, 25, " +
    scene3 +
    ");}";
  var scene1 =
    "function(){generateAnimatedSequence(0, 120, 1, 4, 25, " + scene2 + ");}";
  eval("(" + scene1 + ")();");
}
...
function runFadeToBlack() {
  ...
  var scene9 =
    "function(){generateAnimatedSequence(2000, 2250, 80, 90, 25); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 7;}";
  var scene8 =
    "function(){generateAnimatedSequence(1750, 2000, 70, 80, 25, " +
    scene9 +
    "); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 6;}";
  var scene7 =
    "function(){generateAnimatedSequence(1500, 1750, 60, 70, 25, " +
    scene8 +
    "); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 5;}";
  var scene6 =
    "function(){generateAnimatedSequence(1250, 1500, 50, 60, 25, " +
    scene7 +
    "); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 4;}";
  var scene5 =
    "function(){generateAnimatedSequence(1000, 1250, 40, 50, 25, " +
    scene6 +
    "); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 3;}";
  var scene4 =
    "function(){generateAnimatedSequence(750, 1000, 30, 40, 25, " +
    scene5 +
    "); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 2;}";
  var scene3 =
    "function(){generateAnimatedSequence(500, 750, 20, 30, 25, " +
    scene4 +
    "); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 1;}";
  var scene2 =
    "function(){generateAnimatedSequence(250, 500, 10, 20, 25, " +
    scene3 +
    "); worldAtDepth(1).workersHired = 10; worldAtDepth(1).workerLevel = 0;}";
  var scene1 =
    "function(){generateAnimatedSequence(0, 250, 1, 10, 25, " +
    scene2 +
    "); worldAtDepth(1).workersHired = 5; worldAtDepth(1).workerLevel = 0;}";
  eval("(" + scene1 + ")();");
}

function generateAnimatedSequence(
  startFrameNumber,
  endFrameNumber,
  startRenderDepth,
  endRenderDepth,
  fps,
  finalCallback = "function(){console.log('done');}",
) {
  var callbackChain = "(";
  for (var i = startFrameNumber; i < endFrameNumber; i++) {
    var renderDepthFloat =
      startRenderDepth +
      ((i - startFrameNumber) / (endFrameNumber - startFrameNumber)) *
        (endRenderDepth - startRenderDepth);
    callbackChain +=
...
function generateSeamlessWorldImage(
  ...
  fs.writeFile(
    "../Screenshots/videoFrames/" + frameNumber + ".png",
    base64Data,
    "base64",
    function (err) {
      callback();
...
async function testWebmUnpack() {
  var numFrames = 30;
  let videoBlob = await fetch("Shared/Assets/Unused/output.webm").then((r) =>
    r.blob(),
  );
...
function addSuperMinersByName(name = "", amount = 1, level = 1) {
  if (superMinerManager !== undefined && superMinerManager !== null) {
    var superMiner = superMinerManager.baseSuperMiners.find(
      (miner) => miner.name === name,
    );
    if (superMiner) {
      var freeSlots =
        superMinerManager.slots - superMinerManager.currentSuperMiners.length;
      if (freeSlots < amount) {
        console.warn(
          "Not enough free slots to add " +
            amount +
            " super miners. Only " +
            freeSlots +
            " slots available.",
      ...
      for (var i = 0; i < amount; i++) {
        ...
        superMinerManager.currentSuperMiners[
          superMinerManager.currentSuperMiners.length - 1
        ].level = level;
      }
    } else {
...
function addBattleEquipsByName(name = "", amount = 1, level = 1) {
  if (battleEquipsManager !== undefined && battleEquipsManager !== null) {
    var battleEquip = battleEquipsManager.baseEquips.find(
      (equip) => equip.name === name,
    );
    if (battleEquip) {
      var freeSlots =
        battleEquipsManager.slots - battleEquipsManager.activeEquips.length;
      if (freeSlots < amount) {
        console.warn(
          "Not enough free slots to add " +
            amount +
      ...
      for (var i = 0; i < amount; i++) {
        ...
        battleEquipsManager.activeEquips[
          battleEquipsManager.activeEquips.length - 1
        ].level = level;
      }
    } else {
